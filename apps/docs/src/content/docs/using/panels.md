...
A panel id with no registered plugin is skipped. So an Atari project shows Memory / Registers / Variables / Output, in that order, while an NES project additionally shows the PPU viewer and the Genesis a VDP viewer. See the [per-machine default panels](/docs/reference/machines/#default-panels).
