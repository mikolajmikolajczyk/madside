 ...
 *  signed-in shows the username + an Import button (open the GitHub panel) and
 *  sign-out. Renders nothing when GitHub isn't configured for this build. Used in
 *  the Welcome header (and reusable elsewhere). `onManage` opens the GitHub panel. */
  ...
  if (gh.signedIn && gh.user) {
    return (
      <div className="gh-acct">
        ...
        {onManage && (
          <button type="button" className="gh-acct__btn" onClick={onManage} title="Import projects / courses from GitHub">
            Import…
          </button>
        )}
