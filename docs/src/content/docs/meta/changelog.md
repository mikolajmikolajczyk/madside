...
- **Radicle issues.** Each release maps to a `milestone:*` on the [project's issues](/docs/meta/project/). Reading the closed issues for a milestone tells you what that release set out to do:

  ```sh
  rad issue list --label milestone:v0.8.0
  ```
...
If a curated, human-readable changelog is added later, it will appear here. Until then, the tags and Radicle milestones are the authoritative record — this page intentionally does not list release notes that could fall out of sync with them.
