...
describe('project templates', () => {
  ...
  it('lists the bundled templates with display metadata, ordered', () => {
    ...
    expect(list.map((t) => t.id)).toEqual(['atari-hello', 'nes-hello', 'atari-130xe-bank', 'nes-apu-hello', 'nes-c-hello', 'atari-c-hello', 'c64-c-hello', 'c64-hello', 'empty', 'genesis-asm-hello', 'zx-asm-hello', 'zx-c-hello'])
    ...
    expect(list[6]).toMatchObject({ id: 'c64-c-hello', machine: 'c64' })
    expect(list[7]).toMatchObject({ id: 'c64-hello', machine: 'c64' })
    expect(list[8]).toMatchObject({ id: 'empty', machine: 'atari-xl' })
    expect(list[9]).toMatchObject({ id: 'genesis-asm-hello', machine: 'genesis' })
    expect(list[10]).toMatchObject({ id: 'zx-asm-hello', machine: 'zx-spectrum' })
    expect(list[11]).toMatchObject({ id: 'zx-c-hello', machine: 'zx-spectrum' })
