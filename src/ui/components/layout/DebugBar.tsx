...
export function DebugBar(p: Props) {
  return (
    <div className="dbgbar">
      <IconBtn label="Build (Ctrl+B)" onClick={p.onAssemble} disabled={p.busy}>
      ...
      <IconBtn label="Run (F5)" onClick={p.onRun} disabled={!p.canRun || p.running} highlight>
        <svg viewBox="0 0 16 16" width={14} height={14}><path d="M4 3 L13 8 L4 13 Z" fill="currentColor"/></svg>
      </IconBtn>
      <IconBtn label="Pause (F6)" onClick={p.onPause} disabled={!p.running}>
      ...
      <IconBtn label="Stop (Shift+F5)" onClick={p.onStop} disabled={!p.hasEmu}>
        <svg viewBox="0 0 16 16" width={14} height={14}><rect x="4" y="4" width="8" height="8" fill="currentColor"/></svg>
      </IconBtn>
      <IconBtn label="Step (F10)" onClick={p.onStep} disabled={p.running}>
      ...
      <IconBtn label="Frame (F11)" onClick={p.onFrame} disabled={p.running}>
        <svg viewBox="0 0 16 16" width={14} height={14}>
          <path d="M3 3 L9 8 L3 13 Z" fill="currentColor"/>
          <rect x="11" y="3" width="2" height="10" fill="currentColor"/>
        </svg>
      </IconBtn>
      <IconBtn label="Reset (Shift+F5)" onClick={p.onReset}>
        <svg viewBox="0 0 16 16" width={14} height={14}>
      ...
      <div className="dbgbar__sep" />
      <IconBtn label="Toggle breakpoint (F9)" onClick={p.onToggleBp}>
...
function IconBtn({ label, onClick, disabled, highlight, children }: IconBtnProps) {
